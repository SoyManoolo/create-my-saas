...
async def request_password_reset(payload: ResetPasswordRequest, db: AsyncSession = Depends(get_db), users: UserRepository = Depends(get_user_repository)):
    ...
    if user and user.is_active:
        try:
            await service(db, users).send_one_time_token(user, TokenPurpose.PASSWORD_RESET)
...
async def confirm_password_reset(payload: ResetPasswordConfirm, db: AsyncSession = Depends(get_db), users: UserRepository = Depends(get_user_repository)):
    ...
    if not user:
        raise AppError("Reset token is invalid or expired.", code="INVALID_RESET_TOKEN", status_code=400)
...
async def confirm_email_verification(payload: TokenRequest, db: AsyncSession = Depends(get_db), users: UserRepository = Depends(get_user_repository)):
    user = await service(db, users).consume_one_time_token(payload.token, TokenPurpose.EMAIL_VERIFICATION)
    if not user:
        raise AppError("Verification token is invalid or expired.", code="INVALID_VERIFICATION_TOKEN", status_code=400)
    user.email_verified = True
    await users.save(user)
    return Response(status_code=204)

@router.post("/email/resend", status_code=204)
async def resend_email_verification(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db), users: UserRepository = Depends(get_user_repository)):
    if not current_user.email_verified:
        await service(db, users).send_one_time_token(current_user, TokenPurpose.EMAIL_VERIFICATION)
    return Response(status_code=204)
...
async def change_password(payload: ChangePassword, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db), users: UserRepository = Depends(get_user_repository)):
    if not current_user.password_hash or not verify_password(payload.current_password, current_user.password_hash):
        raise AppError("Current password is incorrect.", code="INVALID_CREDENTIALS", status_code=401)
    current_user.password_hash = hash_password(payload.new_password)
    await users.save(current_user)
    await service(db, users).revoke_all(current_user.id)
    return Response(status_code=204)
