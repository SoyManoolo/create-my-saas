...
def _provider_config(provider: str) -> dict[str, str] | None:
    ...
    if not redirect_uri:
        redirect_uri = (
            f"{settings.oauth_callback_base_url.rstrip('/')}"
    ...
    return {
        ...
        "authorization_url": _provider_env(
            provider,
            "AUTHORIZATION_URL",
            defaults["AUTHORIZATION_URL"],
        )
        or "",
        "token_url": _provider_env(
            provider,
            "TOKEN_URL",
            defaults["TOKEN_URL"],
        )
        or "",
        "userinfo_url": _provider_env(
            provider,
            "USERINFO_URL",
            defaults["USERINFO_URL"],
        )
        or "",
...
def _json_request(
    ...
    *,
    ...
    try:
        with urlopen(request, timeout=10) as response:  # nosec B310: trusted config URL.
...
async def exchange_profile(
    ...
    if not isinstance(profile, dict):
        raise AppError(
            "The OAuth provider returned an invalid profile.",
            code="OAUTH_PROVIDER_ERROR",
            status_code=502,
        )

    email, verified = await _get_verified_email(provider, profile, headers)
    if not isinstance(email, str) or not email or not verified:
        raise AppError(
            "The OAuth provider did not provide a verified email address.",
    ...
    if (
        isinstance(provider_account_id, bool)
        or not isinstance(provider_account_id, (str, int))
        or not str(provider_account_id)
    ):
        raise AppError(
    ...
    display_name = profile.get("name") or profile.get("login") or email.split("@", 1)[0]
    return OAuthProfile(
        email=email.lower(),
...
async def _get_verified_email(
    provider: str,
    profile: dict,
    headers: dict[str, str],
) -> tuple[str | None, bool]:
    if provider != "github":
        return profile.get("email"), profile.get("email_verified") is True
    ...
    verified_emails = (
        [entry for entry in emails if isinstance(entry, dict)]
    ...
    if email:
        is_verified = any(
            entry.get("email") == email and entry.get("verified") is True
    ...
    return primary_email, bool(primary_email)
...
async def oauth_callback(
    ...
    verifier = result.scalar_one_or_none()
    await db.commit()
    if not verifier:
        raise AppError(
    ...
    response = RedirectResponse(
        f"{settings.frontend_url.rstrip('/')}/auth/oauth/callback",
        status_code=303,
    )
