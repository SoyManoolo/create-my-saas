...
from modules.organizations.router import require_role
from modules.users.model import MembershipRole, Subscription, User
...
async def set_subscription(org_id: UUID, payload: SubscriptionUpdate, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    await require_role(org_id, user, db, {MembershipRole.OWNER, MembershipRole.BILLING})
    item = (await db.execute(select(Subscription).where(Subscription.organization_id == org_id))).scalar_one_or_none()
    if not item: item = Subscription(organization_id=org_id); db.add(item)
...
async def webhook(provider: str, event: WebhookEvent, webhook_secret: str | None = Header(default=None, alias="X-Webhook-Secret"), db: AsyncSession = Depends(get_db)):
    ...
    if not expected or webhook_secret != expected: raise AppError("Webhook signature is invalid.", code="INVALID_WEBHOOK", status_code=401)
