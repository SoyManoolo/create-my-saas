...
export class RateLimiter {
  ...
  private async consumeRedis(key: string, limit: number, seconds: number): Promise<boolean | undefined> {
    ...
    try {
      ...
      const count = await within(client.eval(incrementScript, { keys: [`${this.config.RATE_LIMIT_PREFIX}:${key}:${window}`], arguments: [String(seconds)] }) as Promise<number>, redisCommandTimeoutMs);
